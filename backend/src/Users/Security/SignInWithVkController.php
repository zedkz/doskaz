 ...
 */
class SignInWithVkController extends AbstractController
{
    /**
     * @Route(path="/vk", methods={"POST"})
     ...
     * @Post(
     *     path="/api/accessToken/vk",
     *     summary="Получение токена доступа через vk",
     *     tags={"Токены"},
     *     @RequestBody(
     *         @JsonContent(
    ...
    public function byToken(Request $request, OauthCredentialsRepository $credentialsRepository, UserRepository $userRepository, Flusher $flusher, UserAuthenticator $authenticator)
    {
        ...
        $response = file_get_contents("https://api.vk.com/method/account.getProfileInfo?v=5.124&access_token=$token");
        ...
        if (!$credential) {
            ...
            $credential = new OauthCredentials($user->id(), 'vkontakte', $profile['id']);
