...
export const getters = {
    videos: ({item}) => item.videos.map(video => ({
                            ...
                            return {
                                ...
                                attributes: subGroup.attributes.map(attribute => {
                                    return {
                                        ...
                                        title: `${attribute.title || ''} ${attribute.subTitle || ''}`.trim(),
                                        value: attributes.zones[zone.key][`attribute${attribute.key}`]
                                    }
                                })
                            }
                        })
...
export const actions = {
    async load({commit, rootGetters, getters}, id) {
        const object = await this.$axios.$get(`/api/objects/${id}`, {
            params: {
                disabilitiesCategory: rootGetters['disabilitiesCategorySettings/currentCategoryValue']
            }
        })
        commit('SET_ID', id)
