<template>
    <div>
        <template v-for="(group, index1) in formGroups">
            <div class="add-object__line --lrg" v-if="group.title" :key="`g-${index1}`"><h5 class="add-object__title">{{ group.title }}</h5></div>

            <template v-for="(subGroup, index2) in group.subGroups">
                <div class="add-object__line" v-if="subGroup.title"><h5 class="add-object__title --label">{{subGroup.title}}</h5></div>
                <attribute-field
