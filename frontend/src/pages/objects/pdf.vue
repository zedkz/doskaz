    ...
    <div class="pdf">
        <div class="pdf__top">
            ...
            <div class="pdf__qr">
                ...
                <qrcode tag="img" :value="qrLink" :options="{width: 500, margin: 0, type: 'svg'}"/>
            </div>
        </div>
        <div class="pdf__info">
        ...
        <div class="pdf__item" v-for="(zone, index) in zones" :key="zone.key">
            ...
            <template v-for="group in formAttributes[zone.group]">
                ...
                <div v-for="subGroup in group.subGroups" :class="{'pdf__item-list': !!subGroup.title}">
                    <span v-if="subGroup.title" class="pdf__item-list__title">{{ subGroup.title }}</span>
                    <div class="pdf__item-line" v-for="attribute in subGroup.attributes">
                        <img :src="require('@/assets/icons/pdf-check.svg')" v-if="object.attributes.zones[zone.key][`attribute${attribute.key}`] === 'yes'"/>
                        <img :src="require('@/assets/icons/pdf-not-prov.svg')" v-else-if="object.attributes.zones[zone.key][`attribute${attribute.key}`] === 'not_provided'"/>
